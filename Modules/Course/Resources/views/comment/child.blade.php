        ...
        <li class="mt-4">
            <div class="d-flex justify-content-between">
                <div class="d-flex align-items-center">
                    <a class="pe-3" href="#">
                        @if ($comment->user->avatar)
                            <img class="w-10 h-10 p-1 rounded" data-src="{{ $comment->user->avatar }}"
                                alt="Bordered avatar">
                        @else
                ...
                <div class="modal fade" id="commentModal" tabindex="-1" aria-labelledby="commentModal-title"
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <livewire:course::comment-modal :course="$course" :parent="$comment->id" />
                    </div>
                </div>
            ...
            @if ($comment->child()->count() > 0)
                <ul class="list-unstyled ps-4 ps-md-5 sub-comment">
                    @include('course::comment.child', ['comments' => $comment->child])
                </ul>
            @endif
        </li>
    @endif
